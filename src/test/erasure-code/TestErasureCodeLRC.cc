...
TEST(ErasureCodeLRC, init_kml)
{
  ...
  parameters["directory"] = ".libs";
  EXPECT_EQ(0, LRC.init(parameters, &cerr));
  EXPECT_EQ(4 + 2 + (4 + 2) / 3, LRC.get_chunk_count());
}
...
TEST(ErasureCodeLRC, encode_decode)
{
...

  {
    ...
    EXPECT_EQ(1, minimum.count(4));
    EXPECT_EQ(1, minimum.count(5));
    EXPECT_EQ(1, minimum.count(6));
  ...
  }
  {
    ...
    EXPECT_EQ(1, minimum.count(0));
    EXPECT_EQ(1, minimum.count(2));
